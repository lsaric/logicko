    ...
    public partial class KreirajRacun : Form
    {
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            if (lstRacun.Items.Count > 0)
            {
                ...
                for (int i = 0; i < lstRacun.Items.Count; i++)
                {
                    sqlupit = "INSERT INTO Proizvod_Racun values (" + int.Parse(lstRacun.Items[i].SubItems[0].Text) + ",(SELECT MAX(ID_racuna) FROM Racun)," + int.Parse(lstRacun.Items[i].SubItems[2].Text) + "  )";
                    Baza.Instance.IzvrsavanjeUpita(sqlupit);
